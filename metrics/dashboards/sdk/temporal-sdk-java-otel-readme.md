...
When using an OpenTelemetry-based reporter, histogram metrics are exported exactly as defined by the SDK — no `_seconds` suffix is appended. This means histogram metric names in this dashboard match the raw SDK metric names and are consistent with Go SDK metric names:
...
Counter and gauge metrics are unaffected and use the same names as the Micrometer dashboard.
