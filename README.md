...
|-----------|-------------|-----------------------------------|
| `PollWorkflowTaskQueue` | `ResourceExhausted` | In almost all cases this means the worker is being throttled. On OSS this is typically an RPS limit (`frontend.namespaceRPS`) or the concurrent long-running request limit (`frontend.namespaceCount`) being exceeded. On Temporal Cloud it can additionally mean throttling on Actions Per Second (APS) or Operations Per Second (OPS) limits — check the Temporal Cloud documentation for details on those limits. To determine the exact cause, check your server-side cluster metrics and look at the `cause` tag — key values to look for are `RpsLimit`, `ConcurrentLimit`, and `SystemExhausted`. On Temporal Cloud also check for `ApsLimit`. |
...
| `PollWorkflowTaskQueue` | `Canceled` | Worker shut down and canceled the in-flight poll context. Expected during graceful shutdown. Elevated rates outside of deployments warrant investigation. |
| `PollActivityTaskQueue` | `ResourceExhausted` | Same as workflow poll — execution bucket RPS exhausted. Activity pollers and workflow pollers share the same namespace quota. High activity poller counts combined with high workflow poller counts can jointly exhaust the limit. |
...
| `PollActivityTaskQueue` | `Canceled` | Worker shutdown canceled the poll. Expected during graceful shutdown. |
...
| `PollWorkflowExecutionUpdate` | `Canceled` | Client canceled the poll, typically because the caller's context was canceled. |
| `GetWorkflowExecutionHistory` (long-poll) | `ResourceExhausted` | Execution bucket RPS exhausted at Priority 5 — the lowest priority. This variant is the first to be shed under load. Reduce polling frequency or increase `frontend.namespaceRPS`. |
| `GetWorkflowExecutionHistory` (long-poll) | `NotFound` | Workflow execution not found. Workflow may have completed and exceeded retention, or wrong workflow/run ID. |
...
| `GetWorkflowExecutionHistory` (long-poll) | `Canceled` | Caller canceled the wait. Expected when the watching client disconnects. |
